<script context="module" lang="ts">
	export const load = async (
		fetch: Fetch,
		id: string
	): Promise<LoadOutput<{ level: CommunityLevel }>> => {
		try {
			...
			if (!level)
				return {
					...
					redirect: `/levels/community${MOBILE ? '/mobile' : ''}`
				}
...
{#if !$mobile || $landscape}
	<LevelScene
		...
		back="/levels/community{MOBILE ? '/mobile' : ''}"
		hasNext={false}
		next="/levels/community{MOBILE ? '/mobile' : ''}"
	/>
{:else}
	<main>
		<BackLink href="/levels/community{MOBILE ? '/mobile' : ''}"
			>{level.name}</BackLink
		>
